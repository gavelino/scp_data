...
public class AuthorshipAnalyzer {
	...
	public static final boolean IGNORE_NO_FIRST_AUTHOR = true;
	static String release = "NO-Release";
	private static Date initialCommitterDate;
	public static void main(String[] args) {
		...
		try {
			calculateAuthorship(repositoryPath, repositoryName, filesInfo, modulesInfo,	fileExtractor, linguistExtractor, gitLogExtractor,aliasHandler, initialCommit);
//			printRepoInfo(repositoryPath, repositoryName, filesInfo, modulesInfo,	fileExtractor, linguistExtractor, gitLogExtractor,aliasHandler);
		} catch (Exception e) {
	...
	private static void printCoverageInTime(String repositoryPath,
		...
		//cal.add(Calendar.MONTH, -24);
 		...
 		for (int i = 0; i < 24; i++) {
			...
			printCoverage(repository, devs, cal, IGNORE_NO_FIRST_AUTHOR);
	...
	private static void calculateAuthorship(String repositoryPath,
			...
			FileInfoExtractor fileExtractor,
			LinguistExtractor linguistExtractor,
			GitLogExtractor gitLogExtractor, NewAliasHandler aliasHandler, String initialCommitSha) throws Exception {
			Map<String, List<String>> moduleMap = null;
			Map<String, LogCommitInfo> commits = gitLogExtractor.execute();
			
			if (!initialCommitSha.isEmpty()){
				commits = removeCommitsBefore(commits, initialCommitSha);
			}
...
//			printAuthorRatio(release, repository, files, true);
//			*** system_authors_ratio.jar *** Print authors ratio by system
//			printAuthorRatio(release, repository, files, false);
	}

	private static void printRepoInfo(String repositoryPath,
			String repositoryName, 
			Map<String, List<LineInfo>> filesInfo, 
			Map<String, List<LineInfo>> modulesInfo,
			FileInfoExtractor fileExtractor,
			...
			List<LogCommitInfo> commitsList = new ArrayList<LogCommitInfo>();
			for (LogCommitInfo logCommitInfo : commits.values()) {
				if (logCommitInfo.getCommitterDate()!=null)
					commitsList.add(logCommitInfo);
//				else
//					System.out.println("No commit date: "+ logCommitInfo.getSha());
			}
			Collections.sort(commitsList, new Comparator<LogCommitInfo>() {

				@Override
				public int compare(LogCommitInfo o1, LogCommitInfo o2) {
					
					return o1.getCommitterDate().compareTo(o2.getCommitterDate());
				}
			});
			Date firstCommitDate = commitsList.get(0).getCommitterDate();
			Date lastCommitDate = commitsList.get(commitsList.size()-1).getCommitterDate();
			
			System.out.printf("%s;%d;%d;%d;%s;%s;%d\n", repositoryName, files.size(), nFiles, commits.size(), firstCommitDate, lastCommitDate, daysBetween(firstCommitDate, lastCommitDate));
	}
	...
	private static void printAuthorRatio(String release, Repository repository, List<NewFileInfo> files, boolean byModule) {
...
//					authorsMap.get(module).add(developer);
		...
		if (byModule){
			...
			System.out.println(repository.getFullName()+";"+devs.size()+";"+authors.size()+";"+((float)authors.size()/devs.size()));
	}
...
//		System.out.println("release;fullname;path;module;usernameId;name;email;firstDevCommit;lastDevCommit;firstDevFileCommit;lastDevFileCommit");
		int devId = 1;
		...
		for (Developer dev : repository.getDevelopers()) {
			...
			for (AuthorshipInfo authorshipInfo : dev.getAuthorshipInfos()) {
				if (authorshipInfo.isDOAAuthor()){
					...
					System.out.format("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n", release, repository.getFullName(), file.getPath(), moduleName, mapDevId.get(dev.getNewUserName()), dev.getName(), dev.getEmail(),
							dev.getFirstCommit().getAuthorDate(), dev.getLastCommit().getAuthorDate(), authorshipInfo.getFirstCommit().getAuthorDate(), authorshipInfo.getLastCommit().getAuthorDate());
				}
			}
		}
	}
	...
	private static void printDevs(List<Developer> mainDevs,
		...
		int totalFiles = getTotalFiles(allDevs, IGNORE_NO_FIRST_AUTHOR);
...
//	private static void printDevs2(List<Developer> mainDevs,
//			List<Developer> allDevs, Map<String, List<String>> moduleMap) {
//		int totalFiles = getTotalFiles(allDevs);
//		//System.out.println("release;devName;devEmail;devFiles;allFiles;ratio;spread;focus");
//		DistributionCalculator dc = new DistributionCalculator(moduleMap);
//		for (Developer developer : mainDevs) {
//			if (developer.getAuthorshipFiles().size()>0){
//				DistributionMetrics dMetrics = dc.getDistibutionMetrics(developer);
//				System.out.format("%s;%s;%s;%d;%d;%d;%f\n", release, developer.getName(), developer.getEmail(), 
//						developer.getAuthorshipFiles().size(), totalFiles, developer.getAuthorshipFiles().size()/(float)totalFiles*100, dMetrics.getSpread(), dMetrics.getFocus());
//			}
//		}
//		
//	}
	...
	private static void printDevs(List<Developer> mainDevs, List<Developer> allDevs) {
		...
		for (Developer developer : mainDevs) {
 			int devFiles = developer.getAuthorshipFiles(IGNORE_NO_FIRST_AUTHOR).size();
 			if (devFiles>0)
				System.out.format("%s;%s;%s;%d;%d\n", release, developer.getName(), developer.getEmail(), devFiles, totalFiles);
		}
	}
	...
	private static void printCoverage(Repository repository, List<String> devsName, Calendar cal, boolean ignoreNoFirstAuthorFiles) {
		int nFiles = getTotalFiles(repository.getDevelopers(), ignoreNoFirstAuthorFiles);
	...
	private static int getTotalFiles(List<Developer> developers, boolean ignoreNoFirstAuthorFiles) {
	...
	private static Map<String, LogCommitInfo> removeCommitsBefore(
			Map<String, LogCommitInfo> commits, String commitSHA) {
		LogCommitInfo baseCommit = commits.get(commitSHA);
		if (baseCommit == null){
			System.err.println("Base Commit on removeCommitsBefore method is null. SHA = "+ commitSHA);
			return commits;
		}
		Map<String, LogCommitInfo> newCommits =  new HashMap<String, LogCommitInfo>(commits);
		for (Entry<String, LogCommitInfo> entry : commits.entrySet()) {
			if (entry.getValue().getCommitterDate().before(baseCommit.getCommitterDate()))
				newCommits.remove(entry.getKey());
		}
		newCommits.remove(commitSHA);
//		System.out.println("INFO: "+ (commits.size()-newCommits.size())+" removed commits");
		return newCommits;
	}
